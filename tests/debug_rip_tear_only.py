"""Render rip_tear at multiple severities / seeds as individual JPGs."""
...
def run(image_path, out_root, seeds, severities, res=512):
    ...
    tensor_to_pil(img).save(base_dir / "_original.jpg", quality=92)

    for seed in seeds:
        for sev in severities:
            gen = torch.Generator()
            gen.manual_seed(seed)
            mask, _ = generate_local_mask(H, W, sev, (1, 3), (0.05, 0.45), generator=gen)
            out = apply_rip_tear(img, mask, generator=gen)
            tensor_to_pil(out).save(
                base_dir / f"seed{seed:03d}_sev{sev:.2f}.jpg", quality=92
            )
    print(f"{stem}: {len(seeds) * len(severities)} images → {base_dir}")
...
def main():
    ...
    out_root = Path("tests/corruption_results/rip_tear_only")
